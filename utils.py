# in this python file major technical functions will be performed such as generating summary, generating statistics, plots, extractions, model training and many more. 
import pandas as pd
import numpy as np
...
def train_and_evaluate_model(user_input: str, df: pd.DataFrame) -> str:
    ...
    try:
        ...
        if model_type == 'regression':
            ...
            target_names_for_report = [label_encoder.inverse_transform([label])[0] for label in labels] if label_encoder else None
            target_names_for_report = [str(label) for label in target_names_for_report]
